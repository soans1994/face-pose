...
class generator(Sequence):
    ...
    def __init__(self, df, base_dir, input_dim, output_dim, shuffle=False, \
...
class generator2(Sequence):
    ...
    def generate_heatmaps(self, label):
        heat_maps = np.zeros((*self.output_dim, NUM_COCO_FACE_KEYPOINTS))
        for i in range(NUM_COCO_FACE_KEYPOINTS):
            label_idx = i * NUM_COCO_KP_ATTRBS  # index for label
            # generate empty heatmap for unlabelled kp
            if label[label_idx + (NUM_COCO_KP_ATTRBS-1)] == 0:
                continue
    ...
    def convert_coco_kp_to_imgaug_kp(self, label):
        kps = []
        valid = np.ones(NUM_COCO_FACE_KEYPOINTS)
        invalid_xy = -1
        for i in range(NUM_COCO_FACE_KEYPOINTS):
            label_idx = i * NUM_COCO_KP_ATTRBS  # index for label
            # generate empty heatmap for unlabelled kp
    ...
    def __getitem__(self, idx):
        ...
        # Order of last dimension: (heatmap for each kp) repeated num_hg_blocks times
        y = np.empty((self.batch_size, *self.output_dim, NUM_COCO_FACE_KEYPOINTS))
        ...
        for i, data_index in enumerate(indices):
            ...
            #     ann['keypoints'], cropped_width, cropped_height, anchor_x, anchor_y)
            transformed_label = self.transform_label(
                ann['face_kpts'], cropped_width, cropped_height, anchor_x, anchor_y)
            ...
            if self.is_eval:
                ...
                metadata["transformed_label"] = transformed_label #DEBUG
                #metadata["ground_truth_keypoints"] = ann['keypoints'] #DEBUG
                metadata["ground_truth_keypoints"] = ann['face_kpts']  # DEBUG
                metadatas.append(metadata)
...
class generator22(Sequence):
    ...
    def generate_heatmaps(self, label):
        ...
        for i in range(NUM_COCO_FACE_KEYPOINTS):
            ...
            #   added for binary masks
            heat_map[heat_map > 0.05] = 1
            heat_map[heat_map < 0.05] = 0
...
class generator3(Sequence): #face uncropped but resized
    ...
    def transform_image(self, img):
        ...
        new_img = cv2.resize(np.array(img), self.input_dim,interpolation=cv2.INTER_LINEAR)
        return new_img, cropped_width, cropped_height

    def transform_label(self, label, cropped_width, cropped_height):
        label = [int(v) for v in label]
        # adjust x/y coords to new resized img
        transformed_label = []
        for x, y, v in zip(*[iter(label)]*NUM_COCO_KP_ATTRBS):
            x = round((x) * self.input_dim[0]/cropped_width) #scale keypoints to 256 from original size (New/Old)*key
            y = round((y) * self.input_dim[1]/cropped_height)
            # validate kps, throw away if out of bounds
            # TODO: if kp is thrown away then we must update num_keypoints
            if (x > self.input_dim[0] or x < 0) or (y > self.input_dim[1] or y < 0):
                x, y, v = (0, 0, 0)
    ...
    def __getitem__(self, idx):
        ...
        for i, data_index in enumerate(indices):
            ...
            # transformed_img, cropped_width, cropped_height, anchor_x, anchor_y = self.transform_image(
            #     img, ann['bbox'])
            transformed_img, cropped_width, cropped_height = self.transform_image(img)
            # transformed_label = self.transform_label(
            #     ann['keypoints'], cropped_width, cropped_height, anchor_x, anchor_y)
            transformed_label = self.transform_label(ann['face_kpts'], cropped_width, cropped_height)
...
class generator3gray(Sequence):
    ...
    def __getitem__(self, idx):
        # Initialize Batch:
        X = np.empty((self.batch_size, *self.input_dim, INPUT_CHANNELS_GRAY))
