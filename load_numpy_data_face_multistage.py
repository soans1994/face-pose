...
class generator(tensorflow.keras.utils.Sequence):
    ...
    def __init__(self, directory, idxs, img_dict, labels_dict,
                 ...
                 transform_dict=None, shuffle=True):#shuffle false for testing same data
...
if __name__=="__main__":
    ...
    plt.show()
    plt.imshow(np.array(train_y[0][0]).reshape(96, 96, 15).sum(axis=-1))#for 2nd
    plt.show()
    plt.imshow(train_y[0][0].reshape(96, 96, 15)[:,:,0])  # 368,368
    plt.show()
    plt.imshow(train_y[0][0].reshape(96, 96, 15)[:,:,14])  # 368,368
    plt.show()