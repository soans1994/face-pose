    ...
    def _transform_image(self, img, keypoints):
        ...
        if self.transform_dict['Rotate']:
            ...
            if self.transform_dict['Scale']:
                s = random.uniform(0.8, 1.2)#0.7, 1.0
            else:
                s = 1.0

            r = random.randint(-10, 10)#-10, 10
...
if __name__=="__main__":
    ...
    n_train = df_train['Image'].size #7049
    print("train csv size is",n_train)
    n_test = df_test['Image'].size #1783
    print("test csv size is", n_test)
    ...
    for i in range(n_train):
        ...
        if True in df_train.iloc[i, 0:30].isna().values: #if missing values
            ...
            kp_dict[i] = kp
    random.shuffle(idxs)
    print("missed samples are",miss_counter)
    # subset = int(0.1*len(idxs))

    print("length of indexes",len(idxs))
    ...
    transform_dict = {"Flip": True, "Shift": False, "Scale": False, "Rotate": False}
    ...
    val_gen = generator(os.path.join(data_dir, train_dir),
                            val_idxs,
                            img_dict,
                            kp_dict,
                            augment=False,
    ...
    print("\n# of training batches= %d" % len(train_gen)) # x5 train sets 668
    print("# of validation batches= %d" % len(val_gen))  # 111
    ...
    plt.show()
    plt.imshow(np.array(train_y[1]).reshape(96, 96, 15).sum(axis=-1))  # for 2nd
    plt.show()
    plt.imshow(train_x[2])
    plt.show()
    plt.imshow(np.array(train_y[2]).reshape(96, 96, 15).sum(axis=-1))  # for 2nd
    plt.show()
    plt.imshow(train_x[3])
    plt.show()
    plt.imshow(np.array(train_y[3]).reshape(96, 96, 15).sum(axis=-1))  # for 2nd
    plt.show()
    plt.imshow(train_x[4])
    plt.show()
    plt.imshow(np.array(train_y[4]).reshape(96, 96, 15).sum(axis=-1))  # for 2nd
    plt.show()
